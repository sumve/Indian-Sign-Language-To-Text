...
def compute_union_bbox(all_landmarks, img_w, img_h):
    ...
    x2 = min(img_w, cx + box_size // 2)
    y2 = min(img_h, cy + box_size // 2)
...
print(f"\nClass: {label}")
print("Press 's' to start capturing | 'c' to continue after checkpoint | 'q' to quit")
...
while count < SAMPLES_PER_CLASS:
    ...
    cv2.putText(frame, f"{label}: {count}/{SAMPLES_PER_CLASS} | {status}",
